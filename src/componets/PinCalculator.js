...
class CalculatorDisplay extends React.Component {
  render() {
    ...
    const match = value.replace(/[0-9]/g, "X")
    
    if (match)
      formattedValue += (/[1-9]/).test(match[0]) ? match[1] : match[0]
    
    return (
      <div {...props} className="calculator-display2">
        <AutoScalingText>{match}</AutoScalingText>
      </div>
    )
  }
}
  ...
  render() {
    ...
    return (
      <PointTarget onPoint={onPress}>
        <button className={`calculator-key2 ${className}`} {...props}/>
      </PointTarget>
    )
  }
}
...
class CalculatorPin extends React.Component {
  state = {
    value: null,
    displayValue: '0',
    operator: null,
...
let y=discount*x;
  ...
  submit = (e)=>{
    ...
    this.props.Pin(displayValue,this.props.id)
  ...
  render() {
    const { displayValue,showdis,total,discount } = this.state
    ...
    return (<Fragment>
      ...
      <div className="calculator">
       ...
       <button className="checkout" onClick={(e) => (this.submit(e))} >Submit</button>
